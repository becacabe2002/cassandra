...
For operators, the useful mental model is: UCS starts with flushed SSTables, groups them into sharded levels, and then
keeps compacting them toward a target size and density. That lets the same table behave more like STCS, LCS, or TWCS
...
UCS has been designed to maximize the speed of compactions, which is crucial for high-density nodes, using a unique
...
In that case, we use the collection of oldest SSTables that keeps every included overlap set under the configured overlap limit, making sure that if an SSTable is included in this compaction, all older ones are also included to maintain time order.
